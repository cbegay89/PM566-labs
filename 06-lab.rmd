---
title: "Lab 06 - Text Mining Cynthia Begay"
...
```
There are 40 different specialties. The representation from each speciality is not evenly distributed. 
---
...
mt_samples %>%
  ...
  geom_col()
```
We see a lot of articles, propositions, and conjunctions which are a normal part of speech. Patient is the first noun we see with n= 22,065. 
---
...
After removing the stopwords, we see there are more medically oriented words like patient, history, procedure, etc. 
...
mt_samples %>%
 unnest_tokens(word, transcription) %>%
  ...
  geom_col()
```
Patient is the most frequently used word that is not a stop word or number. 
---
...
The phrases are more descriptive with tri-grams. Would are able to pick up certain themes based on the specialty or setting. 
```{r}
mt_samples %>%
...
For the word "blood", estimated could be used as "estimated blood...levels", "white blood...cells", "blood signs", "and blood".
...
Which words are most used in each of the specialties. you can use `group_by()` and `top_n()` from `dplyr` to have the calculations be done within each specialty. Remember to remove stopwords. How about the most 5 used words?
 ...
 For the Allergy/Immunology specialty the word allergies is most used. For Autopsy is 1. Bariatrics is gastric. Cardiovascular/pulmonary is 1. Chirpractic is dr. 
```{r}
mt_samples %>% 
